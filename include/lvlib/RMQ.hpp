...
template<class T>
const DTidx RMQ_succinct<T>::Catalan[17][17] = {
...
template<class T>
const DT RMQ_succinct<T>::minus_infinity = INT_MIN;

template<class T>
const char RMQ_succinct<T>::LSBTable256[256] =
    {
...
template<class T>
DTidx RMQ_succinct<T>::lsb(DTsucc v) {
    return LSBTable256[v];
}

template<class T>
const char RMQ_succinct<T>::LogTable256[256] =
    {
...
template<class T>
DTidx RMQ_succinct<T>::log2fast(DTidx v) {
...
template<class T>
const DTsucc RMQ_succinct<T>::HighestBitsSet[8] = {(DTsucc)~0, (DTsucc) ~1, (DTsucc)~3, (DTsucc)~7, (DTsucc)~15, (DTsucc)~31, (DTsucc)~63, (DTsucc)~127};

template<class T>
DTsucc RMQ_succinct<T>::clearbits(DTsucc n, DTidx x) {
    return n & HighestBitsSet[x];
}

template <class T>
DTidx RMQ_succinct<T>::query(DTidx i, DTidx j) {
...
template <class T>
RMQ_succinct<T>::RMQ_succinct(T* a, DTidx n) {
    this->a = a;
    this->n = n;
